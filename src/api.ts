...
export function register(firstName: string, lastName: string, email: string, password: string): Promise<void> {
...
export async function reserve(slotId: string, teamId: string): Promise<SlotConfig> {
...
export async function cancel(slotId: string, teamId: string): Promise<SlotConfig> {
  const { slot } = await req<{ slot: SlotConfig }>('/reservations', {
    method: 'DELETE',
    body: JSON.stringify({ slotId, teamId }),
  })
...
export function adminCreate<T>(entity: Entity, body: unknown): Promise<T> {
  return req<T>(`/admin/${entity}`, { method: 'POST', body: JSON.stringify(body) })
}
export function adminUpdate<T>(entity: Entity, body: unknown): Promise<T> {
  return req<T>(`/admin/${entity}`, { method: 'PUT', body: JSON.stringify(body) })
}
